...
Thanks for helping improve Maytrix Menu. Small, focused changes are easiest to review and validate through the automated build.
...
- Harmless local settings features
...
1. Fork the repository only to prepare a contribution back to Maytrix, then create a descriptive branch.
2. Install the .NET 8 SDK and a compatible BepInEx 5 setup.
3. Set `GorillaTagPath` to your local installation. Without local files, the project uses compile-only package references.
4. Build with `dotnet build MaytrixMenu.sln -c Release -p:TreatWarningsAsErrors=true`.
5. Test controller alignment, readability, cleanup, and every changed control in VR.
